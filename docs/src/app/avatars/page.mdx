...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.dev.facesign.ai/avatars \
...
```typescript {{ title: 'Avatar with custom flow' }}
const { session, clientSecret } = await client.session.create({
  clientReferenceId: 'user_123',
  ...
  flow: {
    nodes: [
      { id: 'start', type: FSNodeType.START, outcome: 'greet' },
      {
        ...
        prompt: "Hi! I'm here to verify your identity. This will only take a moment.",
        outcomes: [{ id: 'next', targetNodeId: 'liveness', condition: 'true' }]
      },
      {
        id: 'liveness',
    ...
    edges: [
      ...
      { id: 'e2', source: 'greet', target: 'liveness' },
...
- [Custom Flows](/flows) - Build sophisticated verification flows that leverage avatar interactions