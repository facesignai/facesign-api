...
Sessions are the core of FaceSign's identity verification platform. A session represents a single identity verification flow powered by FaceSign's node-based flow system, guiding users through custom verification steps like conversational AI interactions, biometric checks, document scanning, and multi-factor authentication. {{ className: 'lead' }}

<Note>
  See API Reference for endpoints: <a href="/api#create-session">Create</a>, <a href="/api#get-session">Retrieve</a>, <a href="/api#list-sessions">List</a>, and <a href="/api#refresh-client-secret">Client secret</a>. For building flows, see <a href="/flows/mapping">Flow Builder → FSFlow Mapping</a>.
</Note>

<Note>
  Related objects: <a href="/sessions/object">The Session object</a> and <a href="/sessions/client-secret">The Client Secret object</a>.
</Note>
...
FaceSign sessions work similarly to how Stripe handles payments. When you integrate FaceSign, you create a session using the API and get a session object plus client secret, save the session to your database for tracking, provide the client secret to the FaceSign frontend, listen for webhooks that inform you about session progress, and update your database when webhooks arrive.

This approach gives you faster performance (no repeated API calls), better reliability (your app doesn't depend on external API availability), enhanced control (all verification data is in your database), and a complete audit trail.
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.dev.facesign.ai/sessions/fs_session_abc123 \
      -H "Authorization: Bearer $FACESIGN_API_KEY"
    ```
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.dev.facesign.ai/sessions/fs_session_abc123/refresh \
      -H "Authorization: Bearer $FACESIGN_API_KEY"
    ```
